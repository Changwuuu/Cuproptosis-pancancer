...
```{r}
mc3_XENA=read.table("mc3.v0.2.8.PUBLICtxt.txt",header = T,sep = '\t',check.names = F)
...
CNV_XENA=read.table("CopyNumber.txt",header = T,sep = '\t',check.names = F)
...
```{r}
mutation=read.table(file = "mutation.txt",sep = "\t",header = T,check.names = F,row.names = 1)
...
```{r}
deletion=read.table(file = "deletion.txt",sep = "\t",header = T,check.names = F,row.names = 1)
...
###using excel to vasualize the cluster distribution
...
###
C_PDHA1=PDHA1[grep(pattern="OV",PDHA1[,42]),] #####Remember Change the cancer type everytime
exprSet=C_PDHA1
...
OV_PDHA1=cor_data_df#####Remember Change the cancer type everytime
OV_PDHA1$type="OV"#####Remember Change the cancer type e
...
PDHA1_LNC=rbind(BLCA_PDHA1,BRCA_PDHA1,KIRC_PDHA1,KIRP_PDHA1,LGG_PDHA1,LIHC_PDHA1,LUAD_PDHA1,OV_PDHA1,PRAD_PDHA1,SKCM_PDHA1,THCA_PDHA1,HNSC_PDHA1)
...
###
C_MTF1=MTF1[grep(pattern="OV",MTF1[,187]),] #####Remember Change the cancer type everytime
exprSet=C_MTF1
...
OV_MTF1=cor_data_df#####Remember Change the cancer type everytime
OV_MTF1$type="OV"#####Remember Change the cancer type 
...
KIRC_GLS=cor_data_df#####Remember Change the cancer type
...
##
C_GLS=GLS[grep(pattern="OV",GLS[,113]),] #####Remember Change the cancer type everytime
exprSet=C_GLS
...
OV_GLS=cor_data_df#####Remember Change the cancer type
OV_GLS$type="OV"#####Remember Change the cancer type everytime
...
OV=read.table(file = "TF/OV.TXT",header = T, sep = "\t")
OV=merge(OV,CU,by="Target")
OV=OV[,1:2]
colnames(OV)[2]="sample"
OV=merge(OV,RNASeq,by="sample")
...
cs_TCGA=XENA_clinical=read.table(file="Cell_senescence_and_cell_cycle/cs_TCGA1.txt",sep = "\t",header = T,check.names = F)
...
cc_TCGA=XENA_clinical=read.table(file="Cell_senescence_and_cell_cycle/cc_TCGA1.txt",sep = "\t",header = T,check.names = F)
